...
import com.project.system.mapper.TestStudentMapper;
...
public class TestStudentServiceImpl extends ServiceImpl<TestStudentMapper, TestStudent> implements TestStudentService {
    ...
    public List<Map<String, Object>> getStudentSubjectScores(String userId) {
        ...
        queryWrapper.lambda().eq(TestStudent::getUserId, userId);
        List<TestStudent> studentScores = this.list(queryWrapper);
        System.out.println("查询到 " + studentScores.size() + " 条考试记录。");

        // 2. 查询所有考试信息
        List<Test> allTests = testMapper.selectList(null);
        ...
        Map<String, Double> examTotalScores = studentScores.stream()
                .collect(Collectors.groupingBy(
                        TestStudent::getTestId,
                        Collectors.summingDouble(TestStudent::getPoint)
                ));
    ...
    /* 6.28 新增 错题集*/
    @Override
    public List<TestStudent> getWrongAnswers(String userId) {
