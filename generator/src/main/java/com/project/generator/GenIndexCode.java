...
public class GenIndexCode {
    ...
    public String genIndex(GenTable genTable, List<GenTableColumn> columnList) {
        ...
        for (GenTableColumn genTableColumn : columnList) {
            ...
            if (genTableColumn.getIsQuery() == 1) {
                if ("下拉框".equals(genTableColumn.getHtmlType())) {
                    builder.append("                <el-col :xs=\"24\" :sm=\"12\" :md=\"8\" :lg=\"8\" :xl=\"8\">\n" + "                    <span class=\"search-title\">").append(genTableColumn.getColumnComment()).append(":</span>\n").append("                    <el-select clearable style=\"margin-top:10px\" size=\"mini\" v-model=\"search.").append(genTableColumn.getJavaType()).append("\" placeholder=\"请选择\">\n");
                    ...
                    builder.append("</el-select>\n" +
                            "                </el-col>\n");
                } else if ("Date".equals(genTableColumn.getJavaType())) {
                    if ("BETWEEN".equals(genTableColumn.getQueryType())) {
                        ...
                        builder.append("                <el-col :xs=\"24\" :sm=\"12\" :md=\"8\" :lg=\"8\" :xl=\"8\">\n" + "                    <span class=\"search-title\">登陆时间:</span>\n" + "                    <el-date-picker size=\"mini\"\n" + "                      style=\"margin-top:10px\"\n" + "                      v-model=\"search.").append(genTableColumn.getJavaField()).append("\"\n").append("                      format=\"yyyy-MM-dd\" \n").append("                      value-format=\"yyyy-MM-dd\" \n").append("                    </el-date-picker>\n").append("                </el-col>\n");
                    }
                } else {
                    builder.append("                <el-col :xs=\"24\" :sm=\"12\" :md=\"8\" :lg=\"8\" :xl=\"8\">\n" + "                    <span class=\"search-title\">").append(genTableColumn.getColumnComment()).append(":</span>\n").append("                    <el-input\n").append("                        style=\"margin-top:10px\"\n").append("                        size=\"mini\"\n").append("                        placeholder=\"请输入").append(genTableColumn.getColumnComment()).append("\"\n").append("                        v-model=\"search.").append(genTableColumn.getJavaField()).append("\">\n").append("                    </el-input>\n").append("                </el-col>\n");
                }
            }
        }
        builder.append("                <el-col :xs=\"24\" :sm=\"12\" :md=\"8\" :lg=\"8\" :xl=\"8\">\n" +
        ...
        builder.append("        <div class=\"table\">\n" +
                ...
                "            </el-table-column>\n");
        for (GenTableColumn genTableColumn : columnList) {
            if (genTableColumn.getIsList() == 1) {
                builder.append("            <el-table-column\n" + "              prop=\"").append(genTableColumn.getJavaField()).append("\"\n").append("              label=\"").append(genTableColumn.getColumnComment()).append("\"\n").append("              width=\"180\"\n").append("              >\n").append("            </el-table-column>\n");
            }
        }
        builder.append("            <el-table-column\n" +
                "              label=\"操作\"\n" +
        ...
        for (GenTableColumn genTableColumn : columnList) {
            ...
            if (genTableColumn.getIsQuery() == 1) {
                if ("Date".equals(genTableColumn.getJavaType())) {
                    ...
                    builder.append("        this.search.").append(genTableColumn.getJavaField()).append(" = \"\"\n");
                }
            }
        }
        ...
        builder.append("      handleCurrentChange(val) {\n" +
                ...
                "        });\n" +
                "      },\n");
        builder.append("      deleteDate(ids) {\n" + "        remove").append(genTable.getClassName()).append("({ids:ids}).then(res => {\n").append("            if(res.code == 1000) {\n").append("              this.$message({\n").append("                type: 'success',\n").append("                message: '删除成功!'\n").append("              });\n").append("              this.query()\n").append("            } else {\n").append("              this.$notify.error({\n").append("                title: '错误',\n").append("                message: res.message\n").append("              });\n").append("            }\n").append("          })\n").append("      },\n").append("    },\n").append("    mounted() {\n").append("      this.query()\n").append("    }\n").append(" }\n").append("</script>\n");
