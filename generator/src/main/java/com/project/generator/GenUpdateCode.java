...
public class GenUpdateCode {
    ...
    public String genUpdate(GenTable genTable, List<GenTableColumn> columnList) {
        ...
        builder.append("<template>\n" + "<div>\n" + "  <el-dialog title=\"编辑").append(genTable.getTableComment()).append("\" width=\"40%\" :destroy-on-close=\"true\" :visible.sync=\"updateVisible\" :before-close=\"handleClose\">\n").append("    <el-form :model=\"form\" :rules=\"rules\" ref=\"ruleForm\">\n").append("      <el-row :gutter=\"10\">\n");
        ...
        builder.append("      </el-row>\n" + "    </el-form>\n" + "    <div slot=\"footer\" class=\"dialog-footer\">\n" + "      <el-button size=\"mini\" type=\"primary\" @click=\"submit\">确 定</el-button>\n" + "      <el-button size=\"mini\" @click=\"handleClose\">取 消</el-button>\n" + "    </div>\n" + "  </el-dialog>\n" + "</div>\n" + "</template>\n" + "\n" + "<script>\n" + "  import {edit").append(genTable.getClassName()).append(",get").append(genTable.getClassName()).append("ById} from '../../../api/api'\n").append("  export default {\n").append("    data() {\n").append("      return{\n").append("        formLabelWidth: '80px',\n").append("        form: {\n");
        for (GenTableColumn genTableColumn : columnList) {
            if (genTableColumn.getIsInsert() == 1 && (!"id".equals(genTableColumn.getJavaField()) && !"createBy".equals(genTableColumn.getJavaField()) && !"createTime".equals(genTableColumn.getJavaField()) && !"updateBy".equals(genTableColumn.getJavaField()) && !"updateTime".equals(genTableColumn.getJavaField()))) {
        ...
        builder.append("        this.$emit(\"updateFalse\")\n" +
                ...
                "      \n" +
                "    },\n");
        builder.append("    watch: {\n" + "      updateId(newVal) {\n" + "        if(newVal) {\n" + "          get").append(genTable.getClassName()).append("ById({id:newVal}).then(res => {\n").append("            if(res.code == 1000) {\n").append("              this.form = res.data\n");
        for (GenTableColumn genTableColumn :columnList) {
            if ("Integer".equals(genTableColumn.getJavaType())) {
                builder.append("              this.form.").append(genTableColumn.getJavaField()).append(" = res.data.").append(genTableColumn.getColumnName()).append(" + \"\"\n");
            }
        }
        builder.append("            } else {\n" +
                "              this.$notify.error({\n" +
